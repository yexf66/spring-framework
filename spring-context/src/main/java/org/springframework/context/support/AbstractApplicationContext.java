...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	protected ResourcePatternResolver getResourcePatternResolver() {
		return new PathMatchingResourcePatternResolver(this);//创建一个资源模式解析器（其实就算用来解析xml配置文件）
	}
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			//做一些初始化工作，如设置开始时间、设置并校验系统环境变量、设置监听器空集合、事件空集合
			prepareRefresh();
			//创建bean工厂,里面核心的是加载BeanDefinition到容器中
			...
			try {
				...
				//执行全部BeanFactoryPostProcessor，对beanFactory进行修改
				// Invoke factory processors registered as beans in the context.
				invokeBeanFactoryPostProcessors(beanFactory);

				//注册BeanPostProcessor到beanFactory并且排序
				...
				//给子类扩展用，比如springboot用这个初始化了servlet容器
	...
	protected void prepareRefresh() {
		...
		//创建并获取环境对象，验证需要的属性值是否已加入环境对象中
		getEnvironment().validateRequiredProperties();

		// Store pre-refresh ApplicationListeners...
		//将监听器加入到applicationListeners 中 ，当前spring代码这里为空，springboot会使用
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		//这里是一个扩展点，可以自定义属性解析器，比如如何定义自定义的date属性的的解析器，具体方法可以参考 CustomDateEditorRegistrar
		beanFactory.addPropertyEditorRegistrar(new ResourceEditorRegistrar(this, getEnvironment()));

		// Configure the bean factory with context callbacks.
		//添加beanPostProcessor  ApplicationContextAwareProcessor ,用此类来完成某些Aware对象的注入
		beanFactory.addBeanPostProcessor(new ApplicationContextAwareProcessor(this));
		//上一步 addBeanPostProcessor 已经完成这些对象的set注入，所以再使用autowire注入的时候需要将这些接口进行忽略
		...
		//设置几个自动装配的特殊规则，当在进行IOC初始化的时候如果有多个实现，那么就使用指定的对象进行注入
	...
	protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		...
		//如果想添加bfpp有两种方式：
		// 1.继承ClassPathXmlApplicationContext.重写customizeBeanFactory里面调用addBeanFactoryPostProcessor
		// 2.xml文件声明bfpp bean，会在 PostProcessorRegistrationDelegate.invokeBeanFactoryPostProcessors()里面
		// String[] postProcessorNames =beanFactory.getBeanNamesForType(BeanFactoryPostProcessor.class, true, false); 开始加载
